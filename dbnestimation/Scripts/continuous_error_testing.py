...
ALL_PARAMS=[ALL_MST,ALL_DIST,ALL_NOISE_STD,ALL_NSAMPLES,ALL_BURNIN,ALL_URATE,ALL_ALG]
#TESTING PARAMETERS
PARALLEL=False
estimate_gu=True
estimate_g1=True
...
def fan_wrapper(fold='None',n=10,dens=.2,sst=.9):
    ...
    while True:
        try:
            ...
            output['transition']=lm.transitionMatrix4(output['gt'],minstrength=.9)
            if estimate_gu:
                data=lm.drawsamplesLG(output['transition'],nstd=np.double(NOISE_STD), samples=BURNIN+NSAMPLES*URATE)
                data=data[:,BURNIN:]
                if np.max(data) > 1000.:
                    raise NameError()
...
for nodes in np.sort(densities.keys())[1:3]:
    for PARAMS in list(product(*ALL_PARAMS)):
        MST,DIST,NOISE_STD,NSAMPLES,BURNIN,URATE,ALG=PARAMS
        print nodes, ': ----'
        print ''
        z = {}
        ...
        for dens in densities[nodes]:
            ...
            z[dens]={}
            if PARALLEL:
                eqclasses = pool.map(partial(fan_wrapper, n=nodes, dens=dens,sst=MST), range(REPEATS))
                z[dens]['results'] = eqclasses
            else:
                z[dens]['results'] =[]
                for i in range(REPEATS):
                    z[dens]['results'].append(fan_wrapper(n=nodes,dens=dens,sst=MST))
